    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">
            Student {index + 1} of {totalRecords}
          </p>
        </div>
        ...
        <Select value={record.status} onValueChange={onStatusChange}>
          <SelectTrigger
            ...
            aria-label={`Select status for ${record.studentName}`}
          >
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      {record.status === 'late' && (
        <div className="grid gap-2">
          <Label htmlFor={`late-time-${index}`}>Late Arrival Time *</Label>
          <Input
            id={`late-time-${index}`}
            type="time"
          ...
          <p id={`late-time-desc-${index}`} className="text-sm text-muted-foreground">
            Enter the time student arrived
          </p>
        </div>
      )}
      ...
      {(record.status === 'late' || record.status === 'absent') && (
        <div className="grid gap-2">
          <Label htmlFor={`remarks-${index}`}>Remarks (Optional)</Label>
          <Input
            id={`remarks-${index}`}
          ...
          <p id={`remarks-desc-${index}`} className="text-sm text-muted-foreground">
            Add notes about this student's attendance
          </p>
        </div>
      )}
